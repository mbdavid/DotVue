...
namespace DotVue
{
    ...
    internal class StringScanner
    {
        ...
        /// </summary>
        public StringScanner(string source, RegexOptions options)
        {
            this.Source = source;
            this.Index = 0;
        ...
        /// <summary>
        /// Read N chars from current cursor position
        /// </summary>
        public string Read(int length)
        {
            if (this.HasTerminated) return string.Empty;

            var str = this.Source.Substring(this.Index, length);
        ...
        {
            return this.Scan(new Regex((pattern.StartsWith("^") ? "" : "^") + pattern, _options));
        }
        ...
        {
            return this.Scan(new Regex((pattern.StartsWith("^") ? "" : "^") + pattern, _options), group);
        }
